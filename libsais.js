...
function libsais_partial_sorting_scan_left_to_right_8u(T,SA,n,B,c,d){
	var i=0,p=T[--n]<<1|T[n-1]>=T[n],v,x=-1>>>1,iB=B.subarray(1024),dN=B.subarray(512);
...
function libsais_partial_sorting_scan_left_to_right_32s_6k(T,SA,n,k,B,c,d){
	var i=0,p=T[--n]<<1|T[n-1]>=T[n],v,x=-1>>>1,iB=B.subarray(4*k),dN=B.subarray(2*k)
	SA[iB[p]++]=n|1<<31;dN[p]=++d;
	for(c--;i<c;dN[v]=d)
		p=SA[i++],d+=p<0,p&=x,SA[iB[v=T[--p]<<1|T[p-1]>=T[p]]++]=p|(dN[v]!=d)<<31,dN[v]=d,
		p=SA[i++],d+=p<0,p&=x,SA[iB[v=T[--p]<<1|T[p-1]>=T[p]]++]=p|(dN[v]!=d)<<31;
...
function libsais_partial_sorting_shift_markers_8u(SA,B){
	for(var C=B.subarray(1024),c=510,p,q,x=1<<31;c>1;){
...
function libsais_partial_sorting_shift_markers_32s_6k(SA,k,B){
	for(var C=B.subarray(4*k),c=k-1<<1,p,q,x=1<<31;c>1;){
		for(var i=C[c],j=B[c-=2]+3,s=x;i>j;)
			p=SA[--i],s^=q=s^(p&x),SA[i]=p^q,
			p=SA[--i],s^=q=s^(p&x),SA[i]=p^q,
			p=SA[--i],s^=q=s^(p&x),SA[i]=p^q,
...
function libsais_partial_sorting_scan_right_to_left_8u(T,SA,n,B,fs,c,d){
	for(var i=n-fs,j=c+2,p,v,x=-1>>>1,dN=B.subarray(512);i>j;)
...
function libsais_partial_sorting_scan_right_to_left_32s_6k(T,SA,n,k,B,fs,c,d){
	for(var dN=B.subarray(2*k),i=n-fs,j=c+2,p,v,x=-1>>>1;i>j;)
		p=SA[--i],d+=p<0,p&=x,v=T[--p]<<1|T[p-1]>T[p],SA[--B[v]]=p|(dN[v]!=d)<<31,dN[v]=d,
		p=SA[--i],d+=p<0,p&=x,v=T[--p]<<1|T[p-1]>T[p],SA[--B[v]]=p|(dN[v]!=d)<<31,dN[v]=d;
	for(j--;i>j;)
		p=SA[--i],d+=p<0,p&=x,v=T[--p]<<1|T[p-1]>T[p],SA[--B[v]]=p|(dN[v]!=d)<<31,dN[v]=d;
	return d
...
function libsais_partial_sorting_scan_right_to_left_32s_4k(T,SA,n,k,B,d){
	for(var iB=B.subarray(3*k),i=n,p,v;i>1;){
		p=SA[--i];
		if(p>0)SA[i]=0,d+=p>>30,p&=~SUFFIX_GROUP_MARKER,v=T[--p]<<1|T[p-1]>T[p],SA[--iB[T[p]]]=p|(v&1)<<31|(B[v]!==d)<<30,B[v]=d;
		p=SA[--i];
		if(p>0)SA[i]=0,d+=p>>30,p&=~SUFFIX_GROUP_MARKER,v=T[--p]<<1|T[p-1]>T[p],SA[--iB[T[p]]]=p|(v&1)<<31|(B[v]!==d)<<30,B[v]=d;
	}
	for(;i;){
...
function libsais_induce_partial_order_32s_6k(T,SA,n,k,B,fs,lc){
	B.fill(0,2*k,4*k);
	var d=libsais_partial_sorting_scan_left_to_right_32s_6k(T,SA,n,k,B,lc,0);
	libsais_partial_sorting_shift_markers_32s_6k(SA,k,B);
	libsais_partial_sorting_scan_right_to_left_32s_6k(T,SA,n,k,B,fs,lc,d)
}
...
function libsais_renumber_and_mark_distinct_lms_suffixes_32s_4k(SA,n,m){
	...
	for(j+=3;i<j;)r=s,s=SA[i],SAm[(SA[i++]=s&x)>>1]=name|s&r&o,name+=s<0;
	if(name<=m){
		for(i=m,j=m+(n>>1)-3,s=-1;i<j;)
...
function libsais_final_sorting_scan_left_to_right_8u(T,SA,n,B){
	var i=0,j=n-1,o=1<<31,p,iB=B.subarray(1536);
	for(SA[iB[T[j]]++]=j|(T[j-1]<T[j])<<31;i<j;++i){
		p=SA[i];SA[i]=p^o;if(p>0)SA[iB[T[--p]]++]=p|(T[p-(0<p)]<T[p])<<31;
		p=SA[++i];SA[i]=p^o;if(p>0)SA[iB[T[--p]]++]=p|(T[p-(0<p)]<T[p])<<31
	}
	for(j++;i<j;++i){
		p=SA[i];SA[i]=p^o;if(p>0)SA[iB[T[--p]]++]=p|(T[p-(0<p)]<T[p])<<31
...
function libsais_final_sorting_scan_right_to_left_8u(T,SA,n,B){
	for(var i=n,p,x=-1>>>1,iB=B.subarray(1792);i>1;){
...
function libsais_induce_final_order_8u(T,SA,n,bwt,B){
	...
	libsais_final_sorting_scan_left_to_right_8u(T,SA,n,B);
	libsais_final_sorting_scan_right_to_left_8u(T,SA,n,B);
	return 0
}
...
function libsais_merge_compacted_lms_suffixes_32s(T,SA,n,m,f){
	...
	for(i=0,j=m-3,s=SAnm[l=f];i<j;++i)
		...
		SA[++i]||(SA[i]=s,s=SAnm[++l]),
		SA[++i]||(SA[i]=s,s=SAnm[++l]);
	for(j+=3;i<j;++i)SA[i]||(SA[i]=s,s=SAnm[++l])
}
...
function libsais_main_32s(T,SA,n,k,fs){
	if(k>0&&fs>=6*k){
		...
		if(m>1){
			...
			libsais_induce_partial_order_32s_6k(T,SA,n,k,B,f,c);
	...
	if(m>1){
		...
		if(names<m){
			...
			if(!B)B=A=new Int32Array(k+4096)
		}
		libsais_count_suffixes_32s(T,n,k,B);
		libsais_initialize_buckets_end_32s_1k(k,B);
